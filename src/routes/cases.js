...
// GET /api/cases/by-client?type=fysiko|nomiko&id=X
// Returns all cases of a specific client. Used by CaseNew sidebar
// (before a case exists, so cannot use /:id/same-client).
router.get('/by-client', async (req, res) => {
  const orgId = req.user.organization_id;
  const { type, id } = req.query;
  if (!type || !id) return res.status(400).json({ error: 'type + id required' });
  if (!['fysiko', 'nomiko'].includes(type)) return res.status(400).json({ error: 'type must be fysiko|nomiko' });

  const clientId = parseInt(id, 10);
  if (!Number.isInteger(clientId) || clientId <= 0) return res.status(400).json({ error: 'invalid id' });

  const clientCol = type === 'fysiko' ? 'fysiko_prosopo_id' : 'nomiko_prosopo_id';
  ...
  try {
    ...
    console.error('[cases/by-client]', err);
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    ...
    const r = await client.query(
      `UPDATE ypotheseis SET
         ...
         fysiko_prosopo_id       = COALESCE($19, fysiko_prosopo_id),
         nomiko_prosopo_id       = COALESCE($20, nomiko_prosopo_id),
         updated_at              = NOW()
       WHERE aa = $21 AND organization_id = $22
       RETURNING *`,
      [
        b.onomasia_id ?? null,
        b.date_eisagogis ?? null,
        ...
        b.eak ?? null,
        b.arithmos_eisagogikou ?? null,
        b.fysiko_prosopo_id ?? null,
        b.nomiko_prosopo_id ?? null,
        id, orgId,
      ]
    );
