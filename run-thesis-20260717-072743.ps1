...
if ((Get-Location).Path -eq 'C:\thesis-web-repo') {
    ...
    Write-Host "==================================================================="
    Write-Host "1. npm install (for nodemailer)"
    ...
    Write-Host "   Cron:  CRON_SECRET = RANDOM_STRING"
    Write-Host "3. git add -A ; git commit -m ""final v3"" ; git push origin main"
    Write-Host "4. Setup daily cron in Railway Cron tab:"
    Write-Host "   POST /api/cron/trial-reminders      with X-CRON-KEY header"
    Write-Host "   POST /api/cron/expire-subscriptions with X-CRON-KEY header"
    Write-Host "   POST /api/cron/execute-deletions    with X-CRON-KEY header"
    exit 0
}
